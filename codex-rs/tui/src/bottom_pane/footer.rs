...
pub(crate) fn goal_status_indicator_line(
    ...
    let label = match indicator {
        ...
        GoalStatusIndicator::Blocked => "目标被阻塞 (/goal resume)".to_string(),
        GoalStatusIndicator::UsageLimited => "目标已达用量上限 (/goal resume)".to_string(),
...
fn quit_shortcut_reminder_line(key: KeyBinding) -> Line<'static> {
    Line::from(vec![key.into(), " 再次按下以退出".into()]).dim()
}

fn esc_hint_line(esc_backtrack_hint: bool) -> Line<'static> {
    let esc = key_hint::plain(KeyCode::Esc);
    if esc_backtrack_hint {
        Line::from(vec![esc.into(), " 再次按下以编辑上一条消息".into()]).dim()
    } else {
        Line::from(vec![
            esc.into(),
            " ".into(),
...
fn shortcut_overlay_lines(state: ShortcutsState) -> Vec<Line<'static>> {
    ...
    lines.push(Line::from(vec!["自定义快捷键：".into(), "/keymap".cyan()]));
    lines
}
