...
/// Returns the path to the naicode configuration directory, which can be
/// specified by the `CODEX_HOME` environment variable. If not set, defaults to
/// `~/.naicode`.
///
/// 注意：环境变量名保留 `CODEX_HOME`（被 arg0/login/config 多处硬编码），
/// 仅默认目录从 `~/.codex` 改为 `~/.naicode`，与官方 codex 隔离。
///
